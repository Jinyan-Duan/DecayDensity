...
void DetectorConstruction::DefineMaterials()
{
  ...
  G4Material * B = man->FindOrBuildMaterial("G4_B");
  G4Material * shui = man->FindOrBuildMaterial("G4_WATER");
  G4double watercontent = 99.88;
  G4double Bcontent = 0.12;
  G4double Bdensity = Bcontent* perCent *2.46 + watercontent * perCent* 1.;//Boron water density
  G4Material* Boronwater = 
  new G4Material("Bwater", Bdensity*g/cm3, 2);
  Boronwater->AddMaterial(B , Bcontent* perCent);
  Boronwater->AddMaterial(shui , watercontent*perCent);
...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  // Absorbers
  //
  fXfront[0] = -0.5*fAbsorSizeX;// minus x position
  //
