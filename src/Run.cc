...
: G4Run(),
  fDetector(detector),
  fParticle(0), fEkin(0.),fGammaCount(0), fTargetXXX(false)
{
...
void Run::EndOfRun(G4bool print) 
{
  ...
  G4int ih0 = 22;
  G4double binWidth0 = analysisManager->GetH1Width(ih0)
                     *analysisManager->GetH1Unit(ih0);
  G4int ih1 = 23;
  G4double binWidth1 = analysisManager->GetH1Width(ih1)
                     *analysisManager->GetH1Unit(ih1);
  G4int ih2 = 24;
  G4double binWidth2 = analysisManager->GetH1Width(ih2)
                     *analysisManager->GetH1Unit(ih2);

  G4double fac0 = (1./(numberOfEvent*binWidth0))*(um/keV);
  G4double fac1 = (1./(numberOfEvent*binWidth1))*(um/keV);
  G4double fac2 = (1./(numberOfEvent*binWidth2))*(um/keV);
  G4double fac = 1./(numberOfEvent);
  G4cout << fac << G4endl;
  // G4double fac0 = (1./(binWidth0))*(um/MeV);
  // G4double fac1 = (1./(binWidth1))*(um/MeV);
  // G4double fac2 = (1./(binWidth2))*(um/MeV);
  analysisManager->ScaleH1(ih0,fac0);
  analysisManager->ScaleH1(ih1,fac1);
  analysisManager->ScaleH1(ih2,fac2);
  
  // analysisManager->ScaleH1(14,fac);
  // analysisManager->ScaleH1(15,fac);
  G4cout<< "numberOfEvent*binWidth2"<<(1./(numberOfEvent*binWidth0))*(um/keV)<<G4endl;
  // analysisManager->ScaleH1(14,fac);
  // analysisManager->ScaleH1(15,fac);
